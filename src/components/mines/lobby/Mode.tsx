...
export default function Mode({ title, loadingGame, handlePlay, children }: Props) {
  return (
    <div className="flex flex-col items-center max-sm:w-[80vw] max-sm:h-72 sm:w-96 sm:h-96 p-6 gap-4 bg-stone-800 rounded-2xl">
      <h4 className="w-full text-xl font-bold text-stone-200">
        {title}
      </h4>
      ...
      <div className="flex flex-col justify-around items-center w-full h-full">
        ...
        <div className="flex gap-2 sm:gap-4">
          ...
          <PlayButton gameType={GameTypes.HAMMER} loadingGame={loadingGame} handlePlay={() => handlePlay(GameTypes.HAMMER)}>
            망치게임
          </PlayButton>
        </div>
      </div>
...
const PlayButton = ({ gameType, loadingGame, handlePlay, children }: { gameType: GameType; loadingGame: GameType; handlePlay: () => void; children: React.ReactNode }) => {
  return (
    <button
      type="button"
      className={cn([
