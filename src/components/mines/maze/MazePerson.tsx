...
export default function MazePerson({ coord, isHolding, setHolding }: Props) {
  return (
    <div className="absolute left-0 top-0 z-10 w-[1px] h-[1px] will-change-transform" style={{ transform: `translate(${coord.x}px, ${coord.y}px)` }}>
      <div className="relative">
        <div className="absolute flex flex-center w-[1px] h-[1px]" style={{ scale: 0.1 }}>
          <div
            ...
            onTouchStart={() => {setHolding(true); console.log('touch start')}}
            onTouchEnd={() => {setHolding(false); console.log('touch end')}}
          ></div>
        </div>
      </div>
    </div>
