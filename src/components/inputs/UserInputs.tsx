...
const FrontInputContainer: React.FC<InputProps> = ({ 
    label,
    labelWidth="w-[80px]",
    helpText,
    children 
        ...
        <S.Layout>
            <div className="flex justify-between items-center w-full">
                {!CommonUtils.isStringNullOrEmpty(label) && (
                    <S.Label 
                        className={`${!CommonUtils.isStringNullOrEmpty(labelWidth) ? labelWidth : ""}`}
                    >
                        {label}
                    </S.Label>
                )}

                {!CommonUtils.isStringNullOrEmpty(helpText) && (
                    <S.HelpText>
                        {helpText}
                    </S.HelpText>
                )}
    ...
    return (
        ...
        >
            ...
            >
                <S.InputBox
                    type={type}
                    onClick={() => {setOptionShow(!isOptionShow)}}
                    $is_active={isOptionShow}
                    $is_error={isError}
                >
                    <span className={`${isSelected ? "active" : ""}`}>
                        {isSelected ? optionNames[value] : placeholder}
                    </span>
...
export const Checkbox = ({
    ...
    return (
        ...
        >
            ...
            <span className="text-sm text-brand_gray-7">
                {label}
            </span>
        </div>
    )
