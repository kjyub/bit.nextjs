...
import { CandleTimes, CandleTimeType, ChartType, ChartTypes } from "./Types";
...
const areaGradient = (ctx: CanvasRenderingContext2D, moreProps: any) => {
...
const ema12 = ema().id(1).options({ windowSize: 12 }).merge((d: IChartData, c: number) => {
    d.ema12 = c;
})
.accessor((d:IChartData) => d.ema12).stroke(EMA12_COLOR)
...
export default function CryptoMarketFinancialChart({ timeType, chartType, candles, getBeforeData }: ICryptoMarketFinancialChart) {
    ...
    const timeTickFormat = useCallback((index: number | Date) => {
    ...
    const onLoadBefore = useCallback(async (start: number | Date, end: number | Date) => {
        if (!chartCanvasRef.current) {
            return
        }
