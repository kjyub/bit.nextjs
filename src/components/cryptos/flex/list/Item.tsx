  ...
  return (
    <Layout>
      <div className="flex flex-col gap-1">
        {/* PNL */}
        <div className={cn(['flex items-baseline gap-2 text-slate-300', { 'text-position-long-strong': pnlRatio > 0, 'text-position-short-strong': pnlRatio < 0 }])}>
        ...
        <PriceInfoBox>
          <dl>
            <dt>평균 매입 가격</dt>
            <dd>{CryptoUtils.getPriceText(flex.position.averagePrice)}{CRYPTO_WALLET_UNIT}</dd>
          </dl>
          <dl>
            <dt>평균 매도 가격</dt>
            <dd>{CryptoUtils.getPriceText(flex.position.averageClosePrice)}{CRYPTO_WALLET_UNIT}</dd>
          </dl>
        </PriceInfoBox>
...
export const FlexItemSkeleton = ({ ref, isShow = true }: { ref?: React.RefObject<HTMLDivElement>, isShow?: boolean }) => {
  return (
