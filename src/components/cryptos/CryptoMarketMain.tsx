...
export default function CryptoMarketMain({ marketCode, marketData, marketCurrent, communityNode }: ICryptoMarket) {
  ...
  const [_openingPrice, setOpeningPrice] = useState<number>(market.openingPrice)
  const [price, setPrice] = useState<number>(marketCurrent.trade_price)
  const [_startPrice, setStartPrice] = useState<number>(marketCurrent.trade_price) // 애니메이션용
  ...
  return (
    <S.MarketLayout>
      ...
      <S.BottomLayout>
        <S.MyTradeLayout>
          <CryptoMyTrade sizeUnitType={sizeUnitType} market={market} marketPrice={price} />
        </S.MyTradeLayout>
